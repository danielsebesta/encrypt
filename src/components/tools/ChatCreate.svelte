<script lang="ts">
  ...
  let password = '';
  let customRoomId = '';
  let selectedMode: ChatRoomMode = 'persistent';
  let roomError = '';

  type ChatRoomMode = 'persistent' | 'ttl-10' | 'ttl-5' | 'burn' | 'live';

  const ROOM_MODES: { value: ChatRoomMode; title: string; hint: string }[] = [
    { value: 'persistent', title: 'chat.modePersistentTitle', hint: 'chat.modePersistentHint' },
    ...
    { value: 'burn', title: 'chat.modeBurnTitle', hint: 'chat.modeBurnHint' },
