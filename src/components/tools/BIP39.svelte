...
<div class="space-y-6 animate-in fade-in duration-500">
  <div class="grid gap-8">
    <div class="grid gap-3">
        <label class="text-xs text-zinc-500 font-bold uppercase tracking-wider text-center">Your 12-word Recovery Phrase</label>
    ...
    <div class="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-xl flex gap-3 items-start">
        ...
        <div class="text-[10px] text-amber-800 dark:text-amber-500 font-medium leading-relaxed uppercase tracking-wider">
            Never share your mnemonic phrase. It grants full access to any associated wallet or encrypted container derived from these bits.
        </div>
    </div>

    <div class="flex gap-3">
        <button on:click={refresh} class="btn flex-1 py-4 uppercase tracking-widest text-xs font-black">Generate New</button>
        <button on:click={copy} class="btn-outline flex-1 py-4 uppercase tracking-widest text-xs font-black">Copy Phrase</button>
    </div>
  </div>
</div>
