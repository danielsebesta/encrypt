<script lang="ts">
  ...
  function confirmPassword() {
    ...
    const link = `${origin}/ledger/${roomId}`;
    sessionStorage.setItem('ledger-password', password.trim());
    sessionStorage.setItem('ledger-share-password', password.trim());
...
<div class="space-y-4">
    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'ledger.customRoomId')}</label>
      <input
        class="input w-full"
        type="text"
        bind:value={customRoomId}
        on:input={handleRoomIdInput}
        placeholder={t(dict, 'ledger.customRoomIdPlaceholder')}
        autocomplete="off"
        data-lpignore="true"
        data-1p-ignore
        data-bwignore="true"
      />
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'ledger.customRoomIdHint')}</p>
    </div>

    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'ledger.password')}</label>
      ...
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'ledger.createDescription')}</p>
    </div>
    <button class="btn w-full" on:click={confirmPassword}>{t(dict, 'ledger.createRoom')}</button>

    <div class="ledger-how-it-works">
      <p class="ledger-how-title">{t(dict, 'ledger.howItWorks')}</p>
      <div class="ledger-how-steps">
        <div class="ledger-how-step">
          <span class="ledger-how-num">1</span>
          <span>{t(dict, 'ledger.howStep1')}</span>
        </div>
        <div class="ledger-how-step">
          <span class="ledger-how-num">2</span>
          <span>{t(dict, 'ledger.howStep2')}</span>
        </div>
        <div class="ledger-how-step">
          <span class="ledger-how-num">3</span>
          <span>{t(dict, 'ledger.howStep3')}</span>
        </div>
      </div>
    </div>
</div>
...
<style>
  ...
  :global(.dark) .ledger-how-it-works {
