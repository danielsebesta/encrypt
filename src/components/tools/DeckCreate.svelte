<script lang="ts">
  ...
  function confirmPassword() {
    ...
    const link = `${origin}/deck/${roomId}`;
    sessionStorage.setItem('deck-password', password.trim());
    sessionStorage.setItem('deck-share-password', password.trim());
...
<div class="space-y-4">
    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'deck.customRoomId')}</label>
      <input
        class="input w-full"
        type="text"
        bind:value={customRoomId}
        on:input={handleRoomIdInput}
        placeholder={t(dict, 'deck.customRoomIdPlaceholder')}
        autocomplete="off"
        data-lpignore="true"
        data-1p-ignore
        data-bwignore="true"
      />
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'deck.customRoomIdHint')}</p>
    </div>

    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'deck.password')}</label>
      ...
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'deck.createDescription')}</p>
    </div>
    <button class="btn w-full" on:click={confirmPassword}>{t(dict, 'deck.createRoom')}</button>

    <div class="deck-how-it-works">
      <p class="deck-how-title">{t(dict, 'deck.howItWorks')}</p>
      <div class="deck-how-steps">
        <div class="deck-how-step">
          <span class="deck-how-num">1</span>
          <span>{t(dict, 'deck.howStep1')}</span>
        </div>
        <div class="deck-how-step">
          <span class="deck-how-num">2</span>
          <span>{t(dict, 'deck.howStep2')}</span>
        </div>
        <div class="deck-how-step">
          <span class="deck-how-num">3</span>
          <span>{t(dict, 'deck.howStep3')}</span>
        </div>
      </div>
    </div>
</div>
