<script lang="ts">
  ...
  function confirmPassword() {
    ...
    const link = `${origin}/memo/${roomId}`;
    sessionStorage.setItem('memo-password', password.trim());
    sessionStorage.setItem('memo-share-password', password.trim());
...
<div class="space-y-4">
    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'memo.customRoomId')}</label>
      <input
        class="input w-full"
        type="text"
        bind:value={customRoomId}
        on:input={handleRoomIdInput}
        placeholder={t(dict, 'memo.customRoomIdPlaceholder')}
        autocomplete="off"
        data-lpignore="true"
        data-1p-ignore
        data-bwignore="true"
      />
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'memo.customRoomIdHint')}</p>
    </div>

    <div class="space-y-1.5">
      <label class="label block">{t(dict, 'memo.password')}</label>
      ...
      <p class="text-[10px] text-zinc-400 dark:text-zinc-500">{t(dict, 'memo.createDescription')}</p>
    </div>
    <button class="btn w-full" on:click={confirmPassword}>{t(dict, 'memo.createRoom')}</button>

    <div class="memo-how-it-works">
      <p class="memo-how-title">{t(dict, 'memo.howItWorks')}</p>
      <div class="memo-how-steps">
        <div class="memo-how-step">
          <span class="memo-how-num">1</span>
          <span>{t(dict, 'memo.howStep1')}</span>
        </div>
        <div class="memo-how-step">
          <span class="memo-how-num">2</span>
          <span>{t(dict, 'memo.howStep2')}</span>
        </div>
        <div class="memo-how-step">
          <span class="memo-how-num">3</span>
          <span>{t(dict, 'memo.howStep3')}</span>
        </div>
      </div>
    </div>
</div>
