  ...
  async function handleFile(e: Event) {
    ...
    if (target.files?.[0]) {
      ...
      if (file.size > MAX_BYTES) {
        error = t(dict, 'tools.exifScrub.imageTooLarge');
        processing = false;
        return;
      }
      error = '';
      try {
        ...
        console.error(e);
        error = t(dict, 'tools.exifScrub.failedToProcess');
        processing = false;
      }
    }
  }
...
<div class="space-y-8 animate-in fade-in duration-500">
  <div class="border-4 border-dashed border-zinc-50 dark:border-zinc-900 p-12 text-center rounded-3xl relative group bg-zinc-50/10 hover:bg-emerald-50/5 hover:border-emerald-500/20 transition-all">
      ...
      <div class="space-y-4">
          ...
          <div>
              <p class="text-base font-bold text-zinc-800 dark:text-zinc-200">{t(dict, 'tools.exifScrub.dropImage')}</p>
              <p class="text-xs text-zinc-500 mt-1 uppercase tracking-widest">{t(dict, 'tools.exifScrub.metadataRemoved')}</p>
          </div>
      </div>
  </div>
  ...
  {#if processing}
      <div class="text-center py-4">
          <span class="text-xs font-bold text-emerald-500 animate-pulse uppercase tracking-[0.2em]">{t(dict, 'tools.exifScrub.redrawing')}</span>
      </div>
  {/if}
  ...
  {#if exifData || scrubbedImageUrl}
      <div class="grid md:grid-cols-2 gap-8 items-start">
          {#if exifData}
              <div class="space-y-4">
                  <h4 class="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-1">{t(dict, 'tools.exifScrub.detectedMetaTags')}</h4>
          ...
          {#if scrubbedImageUrl}
              <div class="space-y-4 animate-in slide-in-from-right-4">
                  <h4 class="text-[10px] font-black uppercase text-emerald-500 tracking-widest px-1">{t(dict, 'tools.exifScrub.privacySafeResult')}</h4>
                  <div class="relative group rounded-2xl overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800">
                      ...
                      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                          <a href={scrubbedImageUrl} download="scrubbed.jpg" class="btn scale-90 group-hover:scale-100 transition-transform">{t(dict, 'tools.exifScrub.downloadJpeg')}</a>
                      </div>
                  </div>
                  <p class="text-[9px] text-zinc-400 text-center italic uppercase">{t(dict, 'tools.exifScrub.newFileGenerated')}</p>
              </div>
          {/if}
      </div>
  {/if}
