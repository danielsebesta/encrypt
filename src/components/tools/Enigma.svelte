...
<div class="space-y-8 animate-in fade-in duration-500">
  <div class="flex justify-center gap-6 md:gap-12 py-4">
      {#each [0, 1, 2] as i}
          <div class="relative group">
              <div class="text-[10px] text-zinc-400 mb-2 text-center uppercase font-bold tracking-tight">Rotor {enigmaRotors[i]}</div>
  ...
  <div class="grid gap-6">
      <div class="grid gap-1.5">
          <label class="text-xs text-zinc-500 font-bold uppercase tracking-wider">Input Stream</label>
          <textarea 
              bind:value={enigmaInput} 
              on:input={handleEnigma}
              placeholder="Type to process through machine..." 
              class="input min-h-[120px] font-mono text-sm uppercase"
          ></textarea>
      </div>
      
      <div class="grid gap-1.5">
          <label class="text-xs text-zinc-500 font-bold uppercase tracking-wider">Scrambled Output</label>
  ...
  <div class="grid grid-cols-2 gap-4">
      <div class="p-4 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 text-[10px] space-y-1">
          <div class="text-zinc-400 uppercase font-black">Reflector</div>
          <div class="font-bold text-zinc-700 dark:text-zinc-300">UTW-B (Historical)</div>
      </div>
      <div class="p-4 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 text-[10px] space-y-1">
          <div class="text-zinc-400 uppercase font-black">Model</div>
          <div class="font-bold text-zinc-700 dark:text-zinc-300">Enigma M3 (Wehrmacht)</div>
      </div>
  </div>
</div>
