<script lang="ts">
  ...
  export let hrefEn: string;
  export let hrefCs: string;
  export let hrefDe: string = '/de/';

  let mobileOpen = false;
  $: dict = getTranslations(locale);
  $: categories = (() => {
    const navCategories = getNavCategories(t, dict);
    const privacy = navCategories.find(cat => cat.name === t(dict, 'nav.privacy'));
    if (privacy) privacy.tools.push({ label: t(dict, 'nav.tool.deadDrop'), href: '/drop' });
...
<nav class="border-b border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl sticky top-0 z-50 shadow-sm dark:shadow-none">
  <div class="max-w-7xl mx-auto px-5 flex items-center justify-between h-16">
    <a href="/" class="flex items-center gap-3 shrink-0 rounded-lg py-1.5 pr-2 -ml-2 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
    ...
    <div class="hidden md:flex items-center gap-2">
            ...
            <div class="absolute top-full left-0 pt-2 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:translate-y-0 group-focus-within:pointer-events-auto transition-all duration-200 z-[60]">
                <div class="w-52 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-xl p-2.5 grid gap-0.5">
                    {#each cat.tools as tool}
                        <a href={tool.href} class="px-3 py-2.5 text-xs font-bold text-zinc-500 dark:text-zinc-400 hover:text-emerald-500 hover:bg-emerald-50/50 dark:hover:bg-emerald-400/5 rounded-xl transition-all">{tool.label}</a>
                    {/each}
                </div>
            </div>
          </div>
      ...
      <div class="flex items-center gap-1">
        ...
        <ThemeToggle {locale} />
        <LanguageToggle currentLocale={locale} hrefEn={hrefEn} hrefCs={hrefCs} hrefDe={hrefDe} />
      </div>
    </div>
    ...
    <div class="flex md:hidden items-center gap-1">
      <ThemeToggle {locale} />
      <LanguageToggle currentLocale={locale} hrefEn={hrefEn} hrefCs={hrefCs} hrefDe={hrefDe} />
      <button on:click={() => mobileOpen = !mobileOpen} class="btn-ghost btn-icon rounded-full p-2.5" aria-label={t(dict, 'nav.menu')}>
        {#if mobileOpen}
  ...
  {#if mobileOpen}
    <div class="md:hidden border-t border-zinc-200 dark:border-zinc-800 max-h-[80vh] overflow-y-auto bg-white dark:bg-zinc-950">
