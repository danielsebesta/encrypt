<script lang="ts">
  ...
  const categories = [
    {
      name: labels?.developer ?? 'Developer',
      tools: [
    ...
    {
      name: labels?.cryptography ?? 'Cryptography',
      tools: [
  ...
  <div class="max-w-7xl mx-auto px-5 flex items-center justify-between h-16">
    ...
    <div class="hidden md:flex items-center gap-2">
      ...
      <div class="flex items-center gap-1">
        ...
        <ThemeToggle />
        <LanguageToggle currentLocale={locale} hrefEn={hrefEn} hrefCs={hrefCs} />
      </div>
    </div>
    ...
    <div class="flex md:hidden items-center gap-1">
      <ThemeToggle />
      <LanguageToggle currentLocale={locale} hrefEn={hrefEn} hrefCs={hrefCs} />
      <button on:click={() => mobileOpen = !mobileOpen} class="btn-ghost btn-icon rounded-full p-2.5" aria-label="Menu">
        {#if mobileOpen}
